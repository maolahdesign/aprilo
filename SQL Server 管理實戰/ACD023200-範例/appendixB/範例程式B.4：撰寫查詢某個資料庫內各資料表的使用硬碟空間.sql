...
THEN (a1.used + ISNULL(a4.used, 0)) - a1.data ELSE 0 END)
* 8 AS index_size,
(CASE WHEN (a1.reserved + ISNULL(a4.reserved, 0)) > a1.used
THEN (a1.reserved + ISNULL(a4.reserved, 0)) - a1.used ELSE 0 END)
* 8 AS unused
FROM
(SELECT object_id, SUM(CASE WHEN (ps.index_id < 2)
...
GROUP BY object_id ) AS a1
INNER JOIN sys.all_objects AS a2
ON a1.object_id = a2.object_id
INNER JOIN sys.schemas AS a3
...
WHERE (it.internal_type IN (202, 204)) GROUP BY it.parent_id
